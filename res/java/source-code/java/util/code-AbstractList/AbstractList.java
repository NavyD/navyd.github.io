    ...
    private class Itr implements Iterator<E> {
        ...
        public void remove() {
            ...
            try {
                ...
                // 如果删除元素不是最后一个元素就将光标设置为当前位置(remove()已经保证删除元素后续元素左移一位了)
                if (lastRet < cursor)
                    cursor--;
                lastRet = -1;
        ...
        /**
         * 关于"快速失败"线程安全的疑问。
         * List.iterator()是new Itr()对象，当前线程在迭代list时，另一个线程list.remove()导致modCount++
         * 更改了modCount的值，但是由于modCount没有同步和设置volatile，另一个线程的修改不应该能马上看到，虽然快速失败
         * 确实不保证一定成功，但是仍然能大概率发生，修改应该看不到才对，这是为何？？？
         *
         *
         */
    }
